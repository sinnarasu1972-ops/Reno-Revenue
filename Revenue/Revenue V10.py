...
# ---------------- CASCADING FILTERS BY DIVISION ----------------

@app.get("/filters-by-division")
def filters_by_division(division: list[str] = Query(None)):
    """Return filter options scoped to the selected divisions (from offline CSV data)."""
...
def _build_export_cols(data):
    """Return ordered column list: base cols → Division/SA/Month/CY → Labour/Spares/TAT."""
...
def _write_cancelled_sheet(writer, canc):
    ...
    if canc.empty:
        ...
        canc_cols = _build_export_cols(canc)
...
def export(
    ...
    export_cols = _build_export_cols(data)

    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        data[export_cols].to_excel(writer, index=False, sheet_name="Invoice Data")
        _style_sheet(writer.sheets["Invoice Data"], highlight_last_n=3)
...
def export_one_pager(
    ...
    def cat(st):
        ...
        if s in ("PS","PM","PAID SERVICE","PAID SERVICES","PM SERVICE","PAID SERVICE (PS)"): return "PS"
...
def export_current_month(
    ...
    gdf_active["Total Revenue"] = gdf_active["Net Taxable Labor Amount"] + gdf_active["Net Taxable Parts Amt"]

    output = io.BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        gdf_active.to_excel(writer, index=False, sheet_name="Current Month")
        _style_sheet(writer.sheets["Current Month"], highlight_last_n=0)
...
def one_pager(
    ...
    def cat(row):
        ...
        if st in ("PS","PM","PAID SERVICE","PAID SERVICES","PM SERVICE","PAID SERVICE (PS)"): return "PS"
...
def current_month(
    ...
    # ---- MTD comparison: use LATEST Invoice Date from Google Sheet as cutoff ----
    # Parse Invoice Date from the Google Sheet rows (use UNFILTERED gdf_all for date detection)
    _gdf_for_date = gdf  # already filtered — use its latest date as MTD cutoff
    if "Invoice Date" in _gdf_for_date.columns:
        raw_gdf_dates = _gdf_for_date["Invoice Date"].astype(str).str.strip().str[:10]
        gdf_dates_parsed = pd.to_datetime(raw_gdf_dates, format="%d-%m-%Y", errors="coerce")
        mask2 = gdf_dates_parsed.isna()
        if mask2.any():
            gdf_dates_parsed[mask2] = pd.to_datetime(raw_gdf_dates[mask2], format="%d/%m/%Y", errors="coerce")
        gdf["_InvDate"] = gdf_dates_parsed
        latest_date = gdf["_InvDate"].dropna().max()
    else:
        from datetime import datetime as _dt
        latest_date = pd.Timestamp(_dt.now())

    if pd.isna(latest_date):
        from datetime import datetime as _dt
        latest_date = pd.Timestamp(_dt.now())

    curr_month_name = latest_date.strftime("%b")          # e.g. "Mar"
    curr_year       = latest_date.year                    # e.g. 2026
    mtd_day         = int(latest_date.day)                # e.g. 18
    month_full      = latest_date.strftime("%B %Y")       # e.g. "March 2026"

    # ---- Normalise service type values for CY25 comparison ----
    # The Google Sheet may use different service type labels than the offline CSV
    # (e.g. sheet uses "BP", CSV uses "B&P" / "BODYSHOP").
    # Solution: map BOTH sides to a canonical category, then filter CY25 by category.
    ...
    def _svc_cat(st):
        ...
        if s in ("3FS",):                                                    return "3FS"
        if s in ("PS","PM","PAID SERVICE","PAID SERVICES","PM SERVICE","PAID SERVICE (PS)"): return "PS"
        if s in ("RR","RUNNING REPAIR"):                                     return "RR"
        if s in ("B&P","BP","BODY & PAINT","BODY AND PAINT","BODYSHOP","BODY SHOP","BODY PAINT"): return "BP"
        if s in ("AMC","AMC SERVICE"):                                       return "AMC"
        return s  # unmapped: use as-is so exact values still work

    # Build the set of CY25 service categories that match the current filter
    # service filter came from the Google Sheet dropdown values
    service_cats = set(_svc_cat(sv) for sv in (service or [])) if service else None

    # For MTD we compare same day-range in CY25 using the offline CSV data
    base_df = df.copy()
    if division and "Division" in base_df.columns:
        base_df = base_df[base_df["Division"].isin(division)]
    if service_cats is not None and "Service Type" in base_df.columns:
        # Map CY25 service types to the same categories and filter
        base_df = base_df[base_df["Service Type"].apply(_svc_cat).isin(service_cats)]
    if sa and "SA Name" in base_df.columns:
        base_df = base_df[base_df["SA Name"].isin(sa)]
    if model and "Model" in base_df.columns:
        base_df = base_df[base_df["Model"].isin(model)]
    ...
    cy26_inflow_val = int(gdf["Repair Order#"].nunique()) if "Repair Order#" in gdf.columns else 0

    # ---- Division-wise summary (CY26 live vs CY25 same MTD period) ----
    div_summary = []
    if "Division" in gdf.columns:
        all_divs = sorted(gdf["Division"].dropna().unique().tolist())
        for div in all_divs:
            dg = gdf[gdf["Division"] == div]
            d_inflow  = int(dg["Repair Order#"].nunique()) if "Repair Order#" in dg.columns else 0
            d_labour  = float(dg["Net Taxable Labor Amount"].sum())
            d_spares  = float(dg["Net Taxable Parts Amt"].sum())
            d_total   = d_labour + d_spares

            # CY25 same MTD range for this division
            cy25_div = cy25_mtd[cy25_mtd["Division"] == div] if "Division" in cy25_mtd.columns else cy25_mtd.iloc[0:0]
            d25_inflow = int(cy25_div["Repair Order#"].nunique()) if "Repair Order#" in cy25_div.columns else 0
            d25_labour = float(cy25_div["Net Taxable Labor Amount"].sum())
            d25_spares = float(cy25_div["Net Taxable Parts Amt"].sum())
            d25_total  = d25_labour + d25_spares
            ...
            div_summary.append({
                ...
                "spares":      d_spares,
                "total":       d_total,
                "cy25_inflow": d25_inflow,
                "cy25_labour": d25_labour,
                "cy25_spares": d25_spares,
                "cy25_total":  d25_total,
                "inflow_pct":  pct(d_inflow, d25_inflow),
                "labour_pct":  pct(d_labour, d25_labour),
                "spares_pct":  pct(d_spares, d25_spares),
                "total_pct":   pct(d_total,  d25_total),
            })

        # Grand total row
        t_i   = sum(r["inflow"]  for r in div_summary)
        t_l   = sum(r["labour"]  for r in div_summary)
        t_s   = sum(r["spares"]  for r in div_summary)
        t_t   = sum(r["total"]   for r in div_summary)
        t25_i = sum(r["cy25_inflow"]  for r in div_summary)
        t25_l = sum(r["cy25_labour"]  for r in div_summary)
        t25_s = sum(r["cy25_spares"]  for r in div_summary)
        t25_t = sum(r["cy25_total"]   for r in div_summary)
        div_summary.append({
            "division":    "TOTAL",
            "inflow":      t_i,   "labour":      t_l,   "spares":      t_s,   "total":      t_t,
            "cy25_inflow": t25_i, "cy25_labour": t25_l, "cy25_spares": t25_s, "cy25_total": t25_t,
            "inflow_pct":  pct(t_i, t25_i), "labour_pct": pct(t_l, t25_l),
            "spares_pct":  pct(t_s, t25_s), "total_pct":  pct(t_t, t25_t),
        })

    return {
        "rows": rows_out.to_dict(orient="records"),
        "summary": {
            "inflow":  cy26_inflow_val,
        ...
        "mtd": {
            "label":         f"{curr_month_name} {curr_year} (1-{mtd_day})",
            "cy26_inflow":   cy26_inflow_val,
            "cy25_inflow":   cy25_mtd_inflow,
            "inflow_pct":    pct(cy26_inflow_val, cy25_mtd_inflow),
...
<div class="container">
  ...
  <div id="page2" class="tab-page">
    ...
    <div class="filter-bar">
      ...
      <button class="export-btn" onclick="exportPage2()">⬇ Export Excel</button>
  ...
  <div id="page3" class="tab-page">
    ...
    <div class="filter-bar">
      ...
      <button class="export-btn" onclick="exportPage3()">⬇ Export Excel</button>
  ...
  <div id="page4" class="tab-page">
    ...
    <div class="filter-bar">
      ...
      <button class="export-btn" onclick="exportPage4()">⬇ Export Excel</button>
  ...
  <div id="page5" class="tab-page">
    ...
    <div class="filter-bar">
      ...
      <button class="export-btn" onclick="exportPage5()">⬇ Export Excel</button>
    ...
    <div id="cm-div-wrap" style="margin-bottom:24px;overflow-x:auto;display:none;">
      <h3 id="cm-div-title" style="font-size:14px;font-weight:700;color:#4f6bdc;margin-bottom:10px;padding-bottom:6px;border-bottom:2px solid #e5e7eb;">
        &#127970; Division-wise Summary
      </h3>
      <table class="comp-table" id="cm-div-table" style="min-width:1100px;">
        <thead>
          <tr id="cm-div-hdr1">
            <th rowspan="2" style="text-align:left;min-width:72px;background:#1e3a8a;">Division</th>
            <th colspan="2" style="background:#4f6bdc;">Inflow (ROs)</th>
            <th rowspan="2" style="background:#2e4baa;min-width:78px;font-size:11px;">Inflow<br>Growth%</th>
            <th colspan="2" style="background:#1d4ed8;">Labour (&#8377;)</th>
            <th rowspan="2" style="background:#1e3a8a;min-width:78px;font-size:11px;">Labour<br>Growth%</th>
            <th colspan="2" style="background:#4f6bdc;">Spares (&#8377;)</th>
            <th rowspan="2" style="background:#2e4baa;min-width:78px;font-size:11px;">Spares<br>Growth%</th>
            <th colspan="2" style="background:#1e3a8a;">Total Revenue (&#8377;)</th>
            <th rowspan="2" style="background:#15803d;min-width:78px;font-size:11px;">Revenue<br>Growth%</th>
          </tr>
          <tr id="cm-div-hdr2">
            <th id="cm-div-cy25-h" style="background:#5a7ae8;color:#fff;font-size:11px;">CY25 MTD</th>
            <th id="cm-div-cy26-h" style="background:#17a34a;color:#fff;font-size:11px;">CY26 MTD</th>
            <th style="background:#5a7ae8;color:#fff;font-size:11px;">CY25 MTD</th>
            <th style="background:#17a34a;color:#fff;font-size:11px;">CY26 MTD</th>
            <th style="background:#5a7ae8;color:#fff;font-size:11px;">CY25 MTD</th>
            <th style="background:#17a34a;color:#fff;font-size:11px;">CY26 MTD</th>
            <th style="background:#5a7ae8;color:#fff;font-size:11px;">CY25 MTD</th>
            <th style="background:#17a34a;color:#fff;font-size:11px;">CY26 MTD</th>
  ...
  <div id="page6" class="tab-page">
    ...
    <div class="filter-bar">
      ...
      <button class="export-btn" onclick="exportPage6()">⬇ Export Excel</button>
...
/* ====================================================
   ...
   Each page has a Division key and dependent keys.
   When Division changes → fetch /filters-by-division → repopulate dependents.
   ==================================================== */

// Map: page-division-key → list of dependent keys to cascade
const CASCADE_MAP = {
    "division":    ["service","sa","invoice","model","month"],
    "c-division":  ["c-service","c-sa","c-invoice","c-model"],
    "d-division":  ["d-service","d-sa","d-invoice","d-model"],
    "op-division": ["op-service","op-sa","op-invoice","op-model","op-month"],
    "cm-division": ["cm-service","cm-sa","cm-model"],
    "d6-division": ["d6-service","d6-sa","d6-invoice","d6-model"],
};

// Map: dependent-filter-key → its API field name
const KEY_TO_FIELD = {
    "service":"service","sa":"sa","invoice":"invoice","model":"model","month":"month",
    "c-service":"service","c-sa":"sa","c-invoice":"invoice","c-model":"model",
    "d-service":"service","d-sa":"sa","d-invoice":"invoice","d-model":"model",
    "op-service":"service","op-sa":"sa","op-invoice":"invoice","op-model":"model","op-month":"month",
    "cm-service":"service","cm-sa":"sa","cm-model":"model",
    "d6-service":"service","d6-sa":"sa","d6-invoice":"invoice","d6-model":"model",
};

async function cascadeFromDivision(divKey) {
    const divVals = Array.from(selections[divKey] || new Set());
    const depKeys = CASCADE_MAP[divKey] || [];

    let url = "/filters-by-division";
    if (divVals.length > 0) {
        const p = new URLSearchParams();
        divVals.forEach(v => p.append("division", v));
        url += "?" + p.toString();
    }
    ...
    try {
        ...
        depKeys.forEach(function(depKey) {
            const field = KEY_TO_FIELD[depKey];
            if (field && data[field]) {
                // Remember currently selected values so we can preserve if still valid
                const prevSel = new Set(selections[depKey] || []);
                fillCustomSelect(depKey, data[field]);
                // Re-check selections that are still in the new list
                const newSet  = new Set(data[field]);
                const toKeep  = [...prevSel].filter(v => newSet.has(v));
                toKeep.forEach(v => {
                    selections[depKey].add(v);
                    const item = document.querySelector(`#list-${depKey} .cs-item[data-value="${CSS.escape(v)}"]`);
                    if (item) {
                        item.classList.add("checked");
    ...
    } catch(e) { console.error("Cascade filter failed:", e); }
}
...
async function loadFilters() {
    try {
        ...
        // Attach cascade listeners to every Division dropdown
        Object.keys(CASCADE_MAP).forEach(function(divKey) {
            _attachDivisionCascade(divKey);
        });

    } catch(e) { console.error("Filter load failed", e); }
}

function _attachDivisionCascade(divKey) {
    // We intercept checkbox changes inside the Division list
    const listEl = document.getElementById("list-" + divKey);
    if (!listEl) return;
    listEl.addEventListener("change", function() {
        // Small delay so the selections Set is updated before we fetch
        setTimeout(function() { cascadeFromDivision(divKey); }, 50);
    });
    // Also cascade when "Select All" or "Deselect All" is clicked
    const panel = document.getElementById("panel-" + divKey);
    if (!panel) return;
    panel.querySelectorAll(".cs-sa-btn").forEach(function(btn) {
        btn.addEventListener("click", function() {
            setTimeout(function() { cascadeFromDivision(divKey); }, 100);
        });
    });
}
...
async function loadCurrentMonth() {
    ...
    document.getElementById("cm-div-wrap").style.display  = "none";
    try {
        ...
        if (mtd) {
            ...
            document.getElementById("cm-mtd-cy26-hdr").innerText = "CY" + String(mtd.mtd_year || 2026).slice(-2) + " MTD (1-" + mtd.mtd_day + ")";

            function mtdBadge(pct) {
                if (pct === null || pct === undefined) return '<span class="pct flat">N/A</span>';
            ...
            var mtdHtml = mtdRows.map(function(row) {
                var cy25v = row[4] ? ("₹ " + row[1]) : Number(row[1]||0).toLocaleString("en-IN");
                var cy26v = row[4] ? ("₹ " + row[2]) : Number(row[2]||0).toLocaleString("en-IN");
                return '<tr>' +
                    '<td style="text-align:left;font-weight:600;">' + row[0] + '</td>' +
                    '<td>' + cy25v + '</td>' +
                    '<td style="color:#15803d;font-weight:700;">' + cy26v + '</td>' +
                    '<td>' + mtdBadge(row[3]) + '</td>' +
                    '</tr>';
            }).join('');
            document.getElementById("cm-mtd-tbody").innerHTML = mtdHtml;
            document.getElementById("cm-mtd-wrap").style.display = "block";
        }

        // ---- Render Division-wise summary ----
        var divData = data.division_summary || [];
        if (divData.length > 0) {
            var cyLabel = mtd ? String(mtd.mtd_year || 2026).slice(-2) : "26";
            var dayLabel = mtd ? " (1-" + mtd.mtd_day + ")" : "";

            // Update sub-header labels
            var cy25h = document.getElementById("cm-div-cy25-h");
            var cy26h = document.getElementById("cm-div-cy26-h");
            if (cy25h) cy25h.innerText = "CY25 MTD" + dayLabel;
            if (cy26h) cy26h.innerText = "CY" + cyLabel + " MTD" + dayLabel;

            // Update section title
            var divTitle = document.getElementById("cm-div-title");
            if (divTitle && mtd) {
                divTitle.innerHTML = '&#127970; Division-wise Summary &mdash; ' + (mtd.label || "") + ' vs CY25 Same Period';
            }

            function divBadge(pct) {
                if (pct === null || pct === undefined) return '<span class="pct flat">N/A</span>';
                var cls  = pct > 0 ? "up" : pct < 0 ? "down" : "flat";
                var sign = pct > 0 ? "▲ +" : pct < 0 ? "▼ " : "";
                return '<span class="pct ' + cls + '">' + sign + pct.toFixed(1) + '%</span>';
            ...
            var divHtml = divData.map(function(r, idx) {
                var isTotal = r.division === "TOTAL";
                var rowStyle = isTotal
                    ? ' style="background:#dde3f8;font-weight:700;border-top:2px solid #4f6bdc;"'
                    : (idx % 2 === 0 ? ' style="background:#f4f6ff;"' : '');
                var divCell = isTotal
                    ? '<td style="text-align:left;font-weight:700;color:#1e3a8a;background:#dde3f8;">TOTAL</td>'
                    : '<td style="text-align:left;font-weight:700;color:#4f6bdc;">' + r.division + '</td>';
                var fw = isTotal ? "700" : "600";
                return '<tr' + rowStyle + '>' +
                    divCell +
                    // Inflow CY25 | CY26 | Growth%
                    '<td>' + Number(r.cy25_inflow||0).toLocaleString("en-IN") + '</td>' +
                    '<td style="color:#15803d;font-weight:' + fw + ';">' + Number(r.inflow||0).toLocaleString("en-IN") + '</td>' +
                    '<td>' + divBadge(r.inflow_pct) + '</td>' +
                    // Labour CY25 | CY26 | Growth%
                    '<td>₹ ' + fmt(r.cy25_labour||0) + '</td>' +
                    '<td style="color:#15803d;font-weight:' + fw + ';">₹ ' + fmt(r.labour||0) + '</td>' +
                    '<td>' + divBadge(r.labour_pct) + '</td>' +
                    // Spares CY25 | CY26 | Growth%
                    '<td>₹ ' + fmt(r.cy25_spares||0) + '</td>' +
                    '<td style="color:#15803d;font-weight:' + fw + ';">₹ ' + fmt(r.spares||0) + '</td>' +
                    '<td>' + divBadge(r.spares_pct) + '</td>' +
                    // Total Revenue CY25 | CY26 | Growth%
                    '<td>₹ ' + fmt(r.cy25_total||0) + '</td>' +
                    '<td style="color:#15803d;font-weight:' + fw + ';">₹ ' + fmt(r.total||0) + '</td>' +
                    '<td>' + divBadge(r.total_pct) + '</td>' +
                    '</tr>';
            }).join('');
...
async function loadDivMonth26() {
    ...
    try {
        ...
        var months    = data.months;   // Only months present in CY26
        var rows      = data.rows;

        if (!divisions.length || !months.length) {
            wrap.innerHTML = '<div class="comp-loading">No CY26 data available.</div>'; return;
        }
        ...
        wrap.innerHTML = '<div class="comp-loading">Error loading data.</div>';
        console.error("DivMonth26 load failed", e);
    }
}

/* ====================================================
   EXPORT HELPERS — PAGES 2-6
   ==================================================== */
async function _triggerExport(url, filename, btnSelector) {
    var btns = document.querySelectorAll(btnSelector || ".export-btn");
    btns.forEach(function(b){ b.disabled = true; b.textContent = "⏳ Exporting…"; });
    try {
        ...
        btns.forEach(function(b){ b.disabled = false; b.textContent = "⬇ Export Excel"; });
    }
}
...
function exportPage2() {
    ...
    _triggerExport("/export-comparison?" + p.toString(), "Renault_CY_Comparison_Export.xlsx");
}
...
function exportPage3() {
    ...
    _triggerExport("/export-division-month?" + p.toString(), "Renault_DivisionMonth_Export.xlsx");
}
...
function exportPage4() {
    ...
    _triggerExport("/export-one-pager?" + p.toString(), "Renault_OnePager_Export.xlsx");
}
...
function exportPage5() {
    ...
    _triggerExport("/export-current-month?" + p.toString(), "Renault_CurrentMonth_Export.xlsx");
}
...
function exportPage6() {
    ...
    _triggerExport("/export-division-month-cy26?" + p.toString(), "Renault_DivMonth_CY26_Export.xlsx");
}
...
function resetPage4() {
    ...
    cascadeFromDivision("op-division");
    loadOnePager();
}
