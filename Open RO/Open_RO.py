...
# =========================================================
DF = pd.DataFrame()
MODEL_COL = None  # detected model name column
...
def load_data():
    ...
    # Detect model column (best match)
    ...
    df["HOLD_REASON_CLEAN"] = df["Hold Reason"].apply(lambda x: safe_str(x, "")).str.strip()
    df.loc[df["HOLD_REASON_CLEAN"] == "", "HOLD_REASON_CLEAN"] = "No reason"

    # Model name clean column
    df["MODEL_NAME_CLEAN"] = df[MODEL_COL].apply(lambda x: safe_str(x, "")).str.strip()
    ...
    DF = df
    print(f"[OK] Loaded rows: {len(DF)} | file: {EXCEL_PATH} | sheet: {SHEET_NAME} | model_col: {MODEL_COL}")
...
def apply_filters(df: pd.DataFrame, args: dict) -> pd.DataFrame:
    ...
    # Date filtering uses RO_DATE_DT
